...
export default class BatchTransactionDownloader {
  ...
  constructor(db) {
    this.db = db;
    this.pid = `BatchTransactionDownloader@${uuid.v4()}`;
  ...
  async getReceipts(transactions) {
    const timer = this.timer.time("download");
    ...
    return withTimeout(
      sendBatch(this.db.web3.rpc.currentProvider, requests),
      5000
    );
  }

  async importTransactions(transactions, receipts) {
    await Promise.all(
      transactions.map((transaction, index) => {
        return this.importTransaction(transaction, receipts[index]);
      })
    );
  }

  async importTransaction(transaction, receipt) {
    try {
      let timer = this.timer.time("postgres");
      await Promise.all([
